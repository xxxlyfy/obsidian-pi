...
**Status: 0.0.40 was published from a passing automated gate (71 test files / 472 tests); the full manual checklist below has not been re-run since 0.0.31.** Keep this checklist current for every release. Do not publish a release until every applicable manual item below passes in the validation vault:
...
- [ ] `npm ci` and `npm run ci` pass for the release candidate (currently 71 test files / 472 tests).
