...
**Status: 0.0.44 was published from a passing automated gate (73 test files / 488 tests); the full manual checklist below has not been re-run since 0.0.31.** Keep this checklist current for every release. Do not publish a release until every applicable manual item below passes in the validation vault:
...
- [ ] `npm ci` and `npm run ci` pass for the release candidate (currently 73 test files / 488 tests).
